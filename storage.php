 ...
 * but WITHOUT PADDING.
 *
 * This is useful for encoding/decoding hash-based URNs such as
 * urn:sha1: OR urn:bitprint:s.
 *
 * @author Robert Kaye
 * @author Gordon Mohr
 * @author TOGoS / Dan Stevens (translated to PHP)
 */
class TOGoS_Base32 {
...
function exit_with_error_message($message) {
	header("HTTP/1.0 500");
	header("Status: 500");
...
if( empty($_SERVER['QUERY_STRING']) ) {
	exit_with_error_message("No query string provided.");
}

if( !preg_match('/^urn:(sha1|bitpring):[A-Z2-7]{32}\b/', $_SERVER['QUERY_STRING'], $bif) ) {
	exit_with_error_message("Query string must be a urn:(sha1|bitprint):[A-Z2-7]{32} you gave: ".$_SERVER['QUERY_STRING']);
}
