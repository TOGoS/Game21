...
export default class SceneShader {
	...
	protected objectToOpacityMap( obj:PhysicalObject, pos:Vector3D ):void {
		...
		eachSubObject(obj, pos, this.game, this.objectToOpacityMap, this);
	}
	
	/**
	 * Pos = position relative to the shademap's origin where this room's origin would appear
	 */
	protected fillRoomOpacityMap( room:Room, pos:Vector3D, game:Game ):void {
		for( let o in room.objects ) {
			const obj = room.objects[o];
			Vector3D.add(pos, obj.position, objPosBuf);
			this.objectToOpacityMap( obj, objPosBuf );
		}
	}
	
	public sceneOpacityMap( roomRef:string, roomPos:Vector3D, game:Game, dest:ShadeRaster ):ShadeRaster {
		dest.data.fill(0);
		...
		this.fillRoomOpacityMap(room, roomPos, game);
		for( const n in room.neighbors ) {
			...
			this.fillRoomOpacityMap(neighbR, roomPosBuf, game);
		}
